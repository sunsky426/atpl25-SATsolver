...
applyGate gate pt@(PT alph qbs) =
  case gate of
    Sing op target_set -> do
      Prelude.mapM_ (VM.modify qbs (evalSingle op)) (S.toList target_set)
      return Nothing
...
-- test :: ST s (VM.MVector s Qubit)
-- test = do
--   v1 <- VM.new 1       -- create vector
--   VM.write v1 0 (qubit 0.5 0.5)
--   v2 <- VM.new 2
--   VM.write v2 0 (qubit 0.5 0.5)
--   res <- simpPureTensor (PT 1 v1) (PT 2 v2)
--   return v

-- -- test2 :: ST s Int
-- test2 :: ST s Int
-- test2 = do           -- call test, which performs the allocation
--   v <- test
--   v2 <- VM.grow v 2
--   let l =  VM.length v2
--   return l
